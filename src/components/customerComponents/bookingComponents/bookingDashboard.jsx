...
import { searchAvailableCars, getCar } from "../../../api/CarApi";
import { modifyBooking, getNextBooking } from "../../../api/BookingApi";
import { getLocationFromId, getAddressFromGeocode } from '../../../api/LocationApi';
...
class BookingDashboard extends Component {
    ...
    handleSubmit = event => {
        ...
        searchAvailableCars(newSearch).then(res => {
    ...
    handlePickupButton() {
        ...
        modifyBooking(this.state.nextBooking)
            .then(() => {
                this.setState({
        ...
        modifyBooking(this.state.nextBooking)
            .then(() => {
                this.setState({
                    ...
                    successMsg: "Car has been returned! Thanks for using DriveFleet!",
                    avaialbleReturn: false
                })
            })
    }
    ...
    componentDidMount() {
        ...
        getNextBooking().then(res => {
            if (Object.keys(res.data).length) {
                ...
                getLocationFromId(res.data.location)
                    .then(res => {
                        getAddressFromGeocode(res.data.address)
                            .then(newRes => {
